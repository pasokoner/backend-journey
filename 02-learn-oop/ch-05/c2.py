...
class War(CardGame):
    ...
    def play(self):
        ...
        self.player2_hand = []
        for i in range(5):
            self.player1_hand.insert(0, self.deck.deal_card())

        for i in range(5):
            self.player2_hand.insert(0, self.deck.deal_card())
    ...
    def __battle(self):
        ...
        while len(self.player1_hand) > 0 or len(self.player2_hand) > 0:
            ...
            if card1 > card2:
                self.player1_hand.append(card1)
                self.player1_hand.append(card2)
                player1_score += 1
                print(f"Player 1 wins with {card1}")
            elif card2 > card1:
                player2_score += 1
                self.player2_hand.append(card1)
                self.player2_hand.append(card2)
